...
class Migrator(object):
    ...
    def stringify(self, data):
        new_data = []
        for d in data:
            new_data.append([str(item) for item in d])
        return new_data

    def migrate(self, model, domain, field_export, mappings=[None], context_out=None, context_in=None):
        header, data = export_data(self.config_export, model, domain, field_export, max_connection=self.export_max_con, batch_size=self.export_batch_size, context=context_out)
        ...
        for mapping in mappings:
            ...
            import_data(self.config_import, model, header=list(to_import_header), data=to_import_data, max_connection=self.import_max_con, batch_size=self.import_batch_size, context=context_in)