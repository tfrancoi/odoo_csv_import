...
processor.process(categ_parent_map, 'data%sproduct.category.parent.csv' % os.sep, {'worker': 1, 'batch_size': 5, 'model': 'product.category'}, 'set')
...
launchfile_write(processor.file_to_write, "3_product_import.sh", python_exe='python-coverage run -a', path='../')
